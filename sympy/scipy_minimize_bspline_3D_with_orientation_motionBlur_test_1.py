...
def computerFeatureProjectionOnImagePlane(Px: Traj1D, Py: Traj1D, Pz: Traj1D, Pyaw: Traj1D, Rc_B, Oc_B, m):
    ...
    t1 = time.time()
    print('diff...')
    m_C_dot = diff(m_C, Traj1D.t) # will take around 1 minute.
    t2 = time.time()
    print('diff finished', (t2-t1))

    print('lambdifing...')
    t3 = time.time()
    args = (Traj1D.t, Traj1D.T, Px.C, Py.C, Pz.C, Pyaw.C)
    m_C_dot_lambdified = lambdify(args, m_C_dot) # will take about 620 seconds
    t4 = time.time()
    print('lambdifing finished', (t4-t3))
    
    Cx_value, T_value = Px.applyOptVect(x)
    Cy_value, T_value = Py.applyOptVect(x)
    Cz_value, T_value = Pz.applyOptVect(x)
    Cyaw_value, T_value = Pyaw.applyOptVect(x)
    t_value = 0.5 * T_value

    m_C_dot_lambdified_value = m_C_dot_lambdified(t_value, T_value, Cx_value, Cy_value, Cz_value, Cyaw_value)
    t5 = time.time()
    for i in range(10000):
        print('m_C_dot_lambdified=', m_C_dot_lambdified_value)
    t6 = time.time()
    print('calling the function 1e5 times was finished', (t6-t5), 1/(t6-t5))

def objectionFunctionWithMotionBlur(x, num, alpha, mc_dot_mag_fun):
    T = x[-1]
    ti = np.linspace(0, T, num, endpoint=True)
    motion_blur_average = mc_dot_mag_fun(x, ti).mean()
    return T + alpha * motion_blur_average
...
def solve_n_D_OptimizationProblem(startPose=[-5, -30, 1, np.pi/3.], endPose=[0, -4, 2.5, np.pi/2.], featuresWorldPosition=np.array([[9.014978e-01, 1.497865e-03, 2.961498],[-8.985040e-01, 1.497627e-03, 2.961498]]), camera_FOV_h=80, camera_FOV_v=60):
    ...
    mc_dot_mag = mc_dot[0]**2 + mc_dot[1]**2
    x_args = Px.C + Py.C + Pz.C + Pyaw.C + [Traj1D.T]
    print('started lambdifiying mc_dot_mag...')
    mc_dot_mag_fun = lambdify((x_args, Traj1D.t), mc_dot_mag, 'numpy')
    print('finished lambdifiying mc_dot_mag...')
    ...
    if not skip_opt:
        ...
        ts = time.time()
        res = optimize.minimize(fun=objectionFunctionWithMotionBlur, args=(100, 100, mc_dot_mag_fun), x0=x_star, method='SLSQP', bounds=bounds, constraints=constraints,
       ...
       -3.99999999e+00, -4.00000000e+00,  1.00000000e+00,  1.00000000e+00,
        ...
        # x_star2 is a solution for the current opt problem.
        x_star2 = np.array([-5.00000000e+00, -5.00000000e+00, -5.00000000e+00, -5.00000000e+00,
       -5.00000000e+00, -4.59686265e+00, -4.26477507e+00, -4.07620557e+00,
       -3.22629800e+00, -1.78752066e+00, -7.09666355e-01, -3.02884483e-01,
       -3.35923001e-19,  3.69798985e-20, -2.96247814e-20,  1.10090446e-25,
       -1.78490785e-26, -3.00000000e+01, -3.00000000e+01, -3.00000000e+01,
       -3.00000000e+01, -3.00000000e+01, -2.91226755e+01, -2.65775695e+01,
       -2.24515493e+01, -1.73062740e+01, -1.19544827e+01, -7.52393815e+00,
       -4.87732403e+00, -3.99999959e+00, -3.99999962e+00, -3.99999964e+00,
       -3.99999966e+00, -3.99999966e+00,  1.00000000e+00,  1.00000000e+00,
        1.00000000e+00,  1.00000000e+00,  1.00000000e+00,  1.47602276e+00,
        2.02676655e+00,  1.84409859e+00,  9.97210527e-01,  4.32618488e-01,
        7.95722843e-01,  1.85459949e+00,  2.49999988e+00,  2.49999990e+00,
        2.49999992e+00,  2.49999993e+00,  2.49999994e+00,  1.04719755e+00,
        1.04719755e+00,  1.04719755e+00,  1.09887708e+00,  1.17060265e+00,
        1.22956630e+00,  1.22217006e+00,  1.05797624e+01])
