...
def objectionFunctionWithMotionBlur(x, num, alpha, m_C_dot_mag_fun):
    T = x[-1]
    ti = np.linspace(0, T, num, endpoint=True)
    motion_blur_average = m_C_dot_mag_fun(x, ti).sum()/num
...
def solve_n_D_OptimizationProblem(startPose=[-5, -30, 1, np.pi/3.], endPose=[0, -4, 2.5, np.pi/2.], featuresWorldPosition=np.array([[9.014978e-01, 1.497865e-03, 2.961498],[-8.985040e-01, 1.497627e-03, 2.961498]]), camera_FOV_h=80, camera_FOV_v=60):
    ...
    # mc_dot_mag = sqrt(mc_dot[0]**2 + mc_dot[1]**2)
    ...
    # T_star = x_star[-1]
    # t = 0.5
    # print(mc_dot_mag_fun(x_star, t))

    # # average_mc_dot_mag_value = mc_dot_mag_fun(x_star, ti_list*T_star).mean()
    # # ob_value = T_star + alpha * average_mc_dot_mag_value
